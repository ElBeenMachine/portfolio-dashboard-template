...
request_body=$(jq -c -r -n --arg title "Release Version $(echo $CI_COMMIT_TAG | head -n 1)" '{title: $title, merge_when_pipeline_succeeds: true, auto_merge_strategy: "merge_when_pipeline_succeeds"}')
