...
export default function GeneralSettings() {
    ...
    useEffect(() => {
        ...
        fetch("/api/config/all").then(res => res.json()).then(data => {
            ...
            // Set the logo
            if (logoRef.current) {
                logoRef.current.value = settings.find((setting: { key: string }) => setting.key === "logo")?.value || "";
            }

            // Set the authentication background
            if (authBgRef.current) {
                authBgRef.current.value = settings.find((setting: { key: string }) => setting.key === "auth-background")?.value || "";
            }
        });
    }, []);
    ...
    return (
        <SettingsSection title="General">
            ...
            <div className="flex flex-col gap-1">
                ...
                <StringInput ref={logoRef} placeholder="https://example.com/logo.png" name={"logo"} />
            </div>

            {/* Authentication Background */}
            <div className="flex flex-col gap-1">
                <label>Authentication Background</label>
                <StringInput ref={authBgRef} placeholder="https://example.com/background.png" name={"auth-background"} />
            </div>
